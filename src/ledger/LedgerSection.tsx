...
import { actionLabel, amountLabel, entryPill, invoiceLabel, KIND_LABEL, SALE_TYPE_LABEL } from "./constants";
...
export default function LedgerSection({ data, accessToken, onReload, showToast, addLog }: Props) {
  ...
  const entryList = (kind: LedgerKind) => {
    ...
    {showEntryForm && <form className="event-form ledger-form" onSubmit={event => void saveEntry(kind, event)}>
      ...
      <select value={entryForm.invoice_status} onChange={event => setEntryForm(form => ({ ...form, invoice_status: event.target.value as EntryForm["invoice_status"] }))}><option value="none">미발행</option><option value="issued">발행 완료</option><option value="received">수금 완료</option></select>
...
function DashboardList({ title, entries, partnerNameById, today, onComplete }: { title: string; entries: LedgerEntry[]; partnerNameById: Map<string, string>; today: string; onComplete: (entry: LedgerEntry) => void }) { return <section className="ledger-dashboard-list"><h3>{title}</h3>{entries.length ? entries.map(entry => { const pill = entryPill(entry, today); return <div className="ledger-row ledger-entry" key={entry.id}><span className={`ledger-pill ledger-pill-${pill.tone}`}>{pill.label}</span><span className="ledger-entry-main"><strong>{entry.title}</strong><small>{partnerNameById.get(entry.partner_id) ?? "거래처"} · 예정 {entry.due_date}</small>{entry.memo.trim() && <small className="ledger-memo">📝 {entry.memo}</small>}</span><AmountCell entry={entry} /><button className="ledger-action" onClick={() => onComplete(entry)}>{actionLabel(entry.kind)}</button></div>; }) : <p>해당 거래가 없습니다.</p>}</section>; }

// 금액 칸: 방향 라벨(받을/받은·지급할/지급한) + 금액 — 두 화면이 같은 표기를 쓰도록 한 컴포넌트로
function AmountCell({ entry }: { entry: LedgerEntry }) { const label = amountLabel(entry); return <strong className="ledger-amount">{label && <span className={`ledger-amount-label${entry.status === "paid" ? " is-done" : ""}`}>{label}</span>}{formatKRW(entry.amount)}</strong>; }
function EntryRow({ entry, partnerName, today, dateEditing, onComplete, onDelete, onEdit, onDateEdit, onDateSave }: { entry: LedgerEntry; partnerName: string; today: string; dateEditing: boolean; onComplete: (entry: LedgerEntry) => void; onDelete: (entry: LedgerEntry) => void; onEdit: () => void; onDateEdit: () => void; onDateSave: (date: string) => void }) { const pill = entryPill(entry, today); return <div className={`ledger-row ledger-entry ${entry.status === "paid" ? "ledger-entry-paid" : ""}`}><span className={`ledger-pill ledger-pill-${pill.tone}`}>{pill.label}</span><span className="ledger-entry-main">{entry.kind === "sale" && entry.sale_type && <em>{SALE_TYPE_LABEL[entry.sale_type]}</em>}<strong>{entry.title}</strong><small>{partnerName}</small></span><span className="ledger-due">{dateEditing ? <input type="date" autoFocus defaultValue={entry.due_date ?? ""} onBlur={event => onDateSave(event.target.value)} onKeyDown={event => { if (event.key === "Enter") onDateSave(event.currentTarget.value); }} /> : <button className="ghost ledger-date-button" onClick={onDateEdit}>{entry.due_date ?? "예정일 없음"}</button>}</span><AmountCell entry={entry} /><span className="ledger-mini-pill">{invoiceLabel(entry.invoice_status, entry.kind)}</span>{entry.status === "open" && <button className="ledger-action" onClick={() => onComplete(entry)}>{actionLabel(entry.kind)}</button>}<button className="ghost" onClick={onEdit}>✏️ 수정</button><button className="danger-ghost" onClick={() => onDelete(entry)}>🗑️ 삭제</button></div>; }
