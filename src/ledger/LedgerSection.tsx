...
import { calcCommission, formatKRW, isDueSoon, isOverdue, todayKST } from "./utils";
...
export default function LedgerSection({ data, accessToken, onReload, showToast, addLog }: Props) {
  ...
  const saveEntry = async (kind: LedgerKind, event: FormEvent) => {
    ...
    const base = Number(entryForm.base_amount);
    const rate = Number(entryForm.commission_rate);
    const amount = isCommission ? calcCommission(base, rate) : Number(entryForm.amount);
  ...
  const saveRule = async (event: FormEvent) => {
    event.preventDefault();
    const input: RuleInput = { kind: ruleForm.kind, partner_id: ruleForm.partner_id, title: ruleForm.title.trim(), amount: Number(ruleForm.amount), day_of_month: Number(ruleForm.day_of_month), memo: ruleForm.memo };
  ...
  const entryList = (kind: LedgerKind) => <>
    ...
    {showEntryForm && <form className="event-form ledger-form" onSubmit={event => void saveEntry(kind, event)}>
      ...
      {kind === "sale" && entryForm.sale_type === "commission" ? <><input type="number" placeholder="기준 금액" value={entryForm.base_amount} onChange={event => setEntryForm(form => ({ ...form, base_amount: event.target.value }))} /><input type="number" step="0.01" placeholder="수수료율 (%)" value={entryForm.commission_rate} onChange={event => setEntryForm(form => ({ ...form, commission_rate: event.target.value }))} /><strong className="ledger-estimate">예상 금액 {formatKRW(calcCommission(Number(entryForm.base_amount) || 0, Number(entryForm.commission_rate) || 0))}</strong></> : <input type="number" placeholder="금액 (원)" value={entryForm.amount} onChange={event => setEntryForm(form => ({ ...form, amount: event.target.value }))} />}
