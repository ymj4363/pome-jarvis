import { formatKRW, isDueSoon, isOverdue, overdueDays, todayKST } from "./utils";
...
export default function LedgerAlerts({ data, onNavigate }: { data: LedgerData | null; onNavigate: () => void }) {
  ...
  return <div className="ledger-alerts">
    ...
    {overdue.slice(0, 3).map(entry => <span key={entry.id}>{entry.title} · {partners.get(entry.partner_id) ?? "거래처"} · D+{overdueDays(entry, today)}</span>)}
